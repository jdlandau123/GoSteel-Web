...
import { forkJoin, from, switchMap, of } from 'rxjs';
...
export class InvoiceService {
  ...
  createInvoice(orderId: string) {
    from(this._firebaseService.getItem('Orders', orderId)).pipe(
      ...
      this._firebaseService.uploadDocument(`invoices/${docName}`, doc.output('blob'));
      this._firebaseService.updateItem('Orders', orderId, {hasInvoice: true});
    });
  }
